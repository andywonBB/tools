...
x=$(hdfs dfs -ls /event/event/$dt)
num_files=$(echo $x | grep -o "tmp" | wc -l)
y=2

# Message to send if a number != 48 is found
error="Oh noes! This is an automated message. Something might be wrong with the event data. I don't see 2 tmp files in the event bucket! A Flume agent may be down"
